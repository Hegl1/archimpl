...
def test_smaller_numbers():
    ...
    comparative_operation = ComparativeOperationExpression(LiteralExpression(comparison_number), ComparativeOperator.SMALLER, ColumnExpression("MatrNr"))
    sum,entries = comparative_helper.evaluate(comparative_operation)
    assert(sum == (entries-2))
    
def test_smaller_equal_numbers():
    comparison_number = 25403
    comparative_operation = ComparativeOperationExpression(LiteralExpression(comparison_number), ComparativeOperator.SMALLER_EQUAL, ColumnExpression("MatrNr"))
    sum,entries = comparative_helper.evaluate(comparative_operation)
    assert(sum == (entries-1))

def test_greater_equal_numbers():
    comparison_number = 26120
    comparative_operation = ComparativeOperationExpression(LiteralExpression(comparison_number), ComparativeOperator.GREATER_EQUAL, ColumnExpression("MatrNr"))
    sum,__annotations__ = comparative_helper.evaluate(comparative_operation)
    assert(sum == 3)

def test_greater_numbers():
    comparison_number = 26120
    comparative_operation = ComparativeOperationExpression(LiteralExpression(comparison_number), ComparativeOperator.GREATER, ColumnExpression("MatrNr"))
    sum,_ = comparative_helper.evaluate(comparative_operation)
    assert(sum == 2)

def test_equal_numbers():
    comparison_number = 29120
    comparative_operation = ComparativeOperationExpression(LiteralExpression(comparison_number), ComparativeOperator.EQUAL, ColumnExpression("MatrNr"))
    sum,_ = comparative_helper.evaluate(comparative_operation)
    assert(sum == 1)

def test_not_equal_numbers():
    comparison_number = 29120
    comparative_operation = ComparativeOperationExpression(LiteralExpression(comparison_number), ComparativeOperator.NOT_EQUAL, ColumnExpression("MatrNr"))
    sum,entries = comparative_helper.evaluate(comparative_operation)
    assert(sum == (entries - 1))
